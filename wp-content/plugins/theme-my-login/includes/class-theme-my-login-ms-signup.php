			...
			switch ( $stage ) {
				case 'validate-user-signup' :
					if ( $active_signup == 'all' || $_POST[ 'signup_for' ] == 'blog' && $active_signup == 'blog' || $_POST[ 'signup_for' ] == 'user' && $active_signup == 'user' ) {
						...
						?>
						<h2><?php printf( __( 'Ви успішно зарєєструвались на сайті під логіном %s' ), $user_name) ?></h2>
						<p><?php _e( 'Але перед початком роботи під ним Вам потрібно буде його ще <strong>активувати зі свого боку</strong>.' ) ?></p>
						<p><?php printf(__( 'Перевірте, будь ласка, Ваш e-mail <strong>%1$s</strong>, туди тільки що повинен був прийти лист з посиланням на цю активізацію.' ),  $user_email) ?></p>
						<?php
				...
				case 'validate-blog-signup':
					if ( $active_signup == 'all' || $active_signup == 'blog' ) {
						...
						?>
						<h2><?php printf( __( 'Вітаємо вас! Ваш сайт, %s, вже майже готовий.' ), "<a href='http://{$domain}{$path}'>{$blog_title}</a>" ) ?></h2>

						<p><?php _e( 'Вам залишилося тільки активувати його та отримати робочій пароль свого акаунту.' ) ?></p>
						<p><?php printf( __( 'Будь ласка, перевірте вказаний Вами e-mail <strong>%s</strong>, туди повинен прийти лист з посиланням, ведуче на активізацію сайту та односчасно на сторінку з автоматично генеруємим робочим паролєм Вашого акаунту.' ),  $user_email) ?></p>
						<?php
						do_action( 'signup_finished' );
					} else {
				...
				case 'gimmeanotherblog':
					...
					?>
					<h2><?php printf( __( 'Вітаємо. Ваш новий сайт, %s відкрит в мережі успішно! '), "<a href='http://{$domain}{$path}'>{$blog_title}</a>" ) ?></h2>
					<p>
						<?php printf( __( 'Його адреса - <a href="http://%1$s">http://%2$s</a>. Щоб керувати їм, Вим можете зайти на нього під своїм основним акаунтом.' ), $domain.$path, $domain.$path, "http://" . $domain.$path . "wp-login.php", $current_user->user_login ) ?>
					</p>
					<?php
					do_action( 'signup_finished' );
					break;
	...
	public function tml_display_activate( &$template ) {
		...
		<?php } else {
			...
			if ( is_wp_error( $result ) ) {
				...
				?>
				<h2><?php _e( 'Ваш акаунт тепер вже активен!' ); ?></h2>

				<div id="signup-welcome">
					<p><span class="h3"><?php _e( 'Ваш логін:' ); ?></span> <?php echo $user->user_login ?></p>
					<p><span class="h3"><?php _e( 'Ваш пароль (тільки що згенерувався автоматично):' ); ?></span> <?php echo $password; ?></p>
				</div>

				<?php if ( $url != network_home_url( '', 'http' ) ) : switch_to_blog( (int) $blog_id ); ?>
					<p class="view"><?php printf( __( 'Ви можете <a href="%1$s">перейти до стрічки свого нового блогу</a>' ), $url, wp_login_url() ); ?></p>
				<?php restore_current_blog(); else: ?>
					<p class="view"><?php printf( __( 'Ви можете <a href="%1$s">зайти на сайт під своїм новим акаунтом</a> чи <a href="%2$s">перейти на свій профайл</a>.' ), network_site_url( 'wp-login.php', 'login' ), network_home_url() ); ?></p>
				<?php endif;
			}
		}
		echo '</div>';
