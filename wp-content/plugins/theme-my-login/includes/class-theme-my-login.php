...
class Theme_My_Login extends Theme_My_Login_Abstract {
	...
	public static function default_pages() {
		return apply_filters( 'tml_default_pages', array(
			'login'        => __( 'Увійти'         ),
			'logout'       => __( 'Вийти'        ),
			'register'     => __( 'Зареєструватися'       ),
			'lostpassword' => __( 'Забули пароль?'  ),
			'resetpass'    => __( 'Відновити пароль' )
		) );
	}
			...
			switch ( $this->request_action ) {
				...
				default:
					...
					if ( ! empty( $_REQUEST['redirect_to'] ) ) {
						$redirect_to = substr( $_REQUEST ['redirect_to'], 0, strlen ($_REQUEST ['redirect_to']) - 9);
					...
					if ( $http_post && isset( $_POST['log'] ) ) {
						...
						if ( ! is_wp_error( $user ) && ! $reauth ) {
							if ( ( empty( $redirect_to ) || $redirect_to == '' || $redirect_to == admin_url() ) ) {
					...
					if		( isset( $_GET['loggedout'] ) && true == $_GET['loggedout'] )
						$this->errors->add( 'loggedout', __( 'Ви тільки що вийшли.' ), 'message' );
					elseif	( isset( $_GET['registration'] ) && 'disabled' == $_GET['registration'] )
						$this->errors->add( 'registerdisabled', __( 'User registration is currently not allowed.' ) );
					elseif	( isset( $_GET['checkemail'] ) && 'confirm' == $_GET['checkemail'] )
						$this->errors->add( 'confirm', __( 'Перевірте e-mail для актівації нового паролю.' ), 'message' );
					elseif ( isset( $_GET['resetpass'] ) && 'complete' == $_GET['resetpass'] )
						$this->errors->add( 'password_reset', __( 'Ваш пароль оновлен.' ), 'message' );
					...
					elseif	( $reauth )
						$this->errors->add( 'reauth', __( 'Для входу заповните, будь ласка, поля.', 'theme-my-login' ), 'message' );
...
<?php
	...
	public static function retrieve_password() {
		...
		if ( empty( $_POST['user_login'] ) ) {
			$errors->add( 'empty_username', __( '<strong>ПОМИЛКА</strong>: Уведіть логін чи e-mail.' ) );
		} else if ( strpos( $_POST['user_login'], '@' ) ) {
			$user_data = get_user_by( 'email', trim( $_POST['user_login'] ) );
			if ( empty( $user_data ) )
				$errors->add( 'invalid_email', __( '<strong>ПОМИЛКА</strong>: Цей e-mail не зарєєстрован на сайті!' ) );
		} else {
		...
		if ( ! $user_data ) {
			$errors->add( 'invalidcombo', __( '<strong>ПОМИЛКА</strong>: Некоректний логін або e-mail.' ) );
			return $errors;
		}
		...
		$message = __( 'Тільки що хтось запросив зміну паролю для наступного сайту:' ) . "\r\n\r\n";
		$message .= network_home_url( '/' ) . "\r\n\r\n";
		$message .= sprintf( __( 'Username: %s' ), $user_login ) . "\r\n\r\n";
		$message .= __( 'Якшо ви не розумієте, про що йде мова, просто проігноруйте цей лист.' ) . "\r\n\r\n";
		$message .= __( 'Якшо же саме Ви запросили пароль, перейдіть, будь ласка, по наступному посиланню:' ) . "\r\n\r\n";
	...
	public static function register_new_user( $user_login, $user_email ) {
		...
		if ( $sanitized_user_login == '' ) {
			$errors->add( 'empty_username', __( '<strong>ПОМИЛКА</strong>: Будь ласка, введить логін.' ) );
		} elseif ( ! validate_username( $user_login ) ) {
			$errors->add( 'invalid_username', __( '<strong>ПОМИЛКА</strong>: This username is invalid because it uses illegal characters. Please enter a valid username.' ) );
			$sanitized_user_login = '';
		} elseif ( username_exists( $sanitized_user_login ) ) {
			$errors->add( 'username_exists', __( '<strong>ПОМИЛКА</strong>: Цей логін вже зареєстрован на сайті, придумайте, будь ласка, іншій.' ) );
		}
		...
		if ( '' == $user_email ) {
			...
			$errors->add( 'email_exists', __( '<strong>ПОМИЛКА</strong>: Цей e-mail вже зарєєстрован на сайті, придумайте, будь ласка, іншій.' ) );
		}
