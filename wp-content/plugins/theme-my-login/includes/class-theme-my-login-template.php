...
class Theme_My_Login_Template extends Theme_My_Login_Abstract {
	...
	public function get_title( $action = '' ) {
		...
		if ( is_user_logged_in() && 'login' == $action && $action == $this->get_option( 'default_action' ) ) {
			$title = sprintf( __( 'Ласкаво просимо, %s', 'theme-my-login' ), wp_get_current_user()->display_name );
		} else {
			if ( $page_id = Theme_My_Login::get_page_id( $action ) ) {
				...
				switch ( $action ) {
					case 'register':
						$title = __( 'Зареєструватись' );
						break;
					case 'lostpassword':
					case 'retrievepassword':
					case 'resetpass':
					case 'rp':
						$title = __( 'Забули пароль?' );
						break;
					case 'login':
					default:
						$title = __( 'Увійти' );
	...
	public static function get_action_template_message( $action = '' ) {
		switch ( $action ) {
			case 'register':
				...
				break;
			case 'lostpassword':
				$message = __( 'Будь ласка, уведіть свій e-mail, на який буде послан Ваш пароль, або логін, який відповідає цьому e-mail.' );
				break;
			case 'resetpass':
				$message = __( 'Будь ласка, придумайте та уведіть свій новий пароль.' );
				break;
			default:
				$message = '';
		}
