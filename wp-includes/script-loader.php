...
function wp_default_scripts( &$scripts ) {
	...
	$uploader_l10n = array(
		...
		'default_error' => __('На даний момент цей файл завантаженню не підлягає. Будь ласка, спробуйте піздніше.'),
	...
	did_action( 'init' ) && $scripts->localize( 'password-strength-meter', 'pwsL10n', array(
		'empty' => __('Індікатор складності паролю'),
		'short' => __('Дуже слабий'),
		'bad' => __('Слабий'),
		/* translators: password strength */
		'good' => _x('Середньої надійньості', 'password strength'),
		'strong' => __('Надійний'),
		'mismatch' => __('Ідеальний')
	) );
	...
	did_action( 'init' ) && $scripts->localize( 'customize-controls', '_wpCustomizeControlsL10n', array(
		...
		'save'      => __( 'Зберігти та опублікувати' ),
		'saved'     => __( 'Збережено' ),
		'cancel'    => __( 'Скасувати' ),
		'close'     => __( 'Закрити' ),
	...
	if ( is_admin() ) {
		...
		did_action( 'init' ) && $scripts->localize( 'wp-color-picker', 'wpColorPickerL10n', array(
			'clear' => __( 'Видалити' ),
			'defaultString' => __( 'За замовчуванням' ),
			'pick' => __( 'Виберіть кольор' ),
			'current' => __( 'Поточний кольор' ),
		) );
