...
final class _WP_Editors {
	 ...
	 */
	public static function editor( $content, $editor_id, $settings = array(), $isSwitch = true, $isPhoto = false, $post = 0, $blog = 0, $goal="")
	{
		...
		if ( self::$this_quicktags && self::$this_tinymce ) {
			...
			if ($isSwitch)
			{
				...
				$buttons .= '<a id="' . $editor_id . '-tmce" class="wp-switch-editor switch-tmce" onclick="switchEditors.switchto(this);">' . __
				('Visual') . "</a>\n";
			}
		}
		...
		if (isPhoto)
			if ($goal == "add")
				$the_editor = apply_filters('the_editor', '<br/><input type="checkbox" id="mode"><span style="font-size: 12px;">Вкажіть позиції майбутніх ілюстрацій посту</span>
                            <label for="new-post-desc">'.wpuf_get_option( 'desc_label', 'wpuf_labels', 'Текст посту' ).'<span class="required">*</span>
                            </label>
                                <div style="float:left;"><div id="wp-' . $editor_id . '-editor-container" class="wp-editor-container"><textarea' . $editor_class . $height . $tabindex . ' cols="40" name="' . $set['textarea_name'] . '" id="' . $editor_id . '" onClick="addUpload(getCaret(this), '.$post.', '.$blog.', 0);">%s</textarea></div><div id="dest"></div>');
			else
				$the_editor = apply_filters('the_editor', '<br/><br/><input type="checkbox" id="mode1"><span style="font-size: 12px;">Вкажіть позиції майбутніх ілюстрацій посту</span>
                            <label for="new-post-desc">'.wpuf_get_option( 'desc_label', 'wpuf_labels', 'Текст посту' ).'<span class="required">*</span>
                            </label>
                                <div style="float:left;"><div id="wp-' . $editor_id . '-editor-container" class="wp-editor-container"><textarea' . $editor_class . $height . $tabindex . ' cols="40" name="' . $set['textarea_name'] . '" id="' . $editor_id . '" onClick="addUpload(getCaret(this), '.$post.', '.$blog.', 1);">%s</textarea></div><div id="dest1"></div>');
		else
	...
	<div id="link-selector">
		...
		<div id="search-panel"<?php if ( ! $show_internal ) echo ' style="display:none"'; ?>>
			<div class="link-search-wrapper">
				<label>
					<span class="search-label"><?php _e( 'Пошук' ); ?></span>
...
function getCaret(el)
  ...
  function addUpload(car, post, blog, goal)
  {
  //alert(car);
  //alert (document.getElementById ("mode").checked);
  if ((goal == 0) && (document.getElementById ("mode").checked))
  {
  var req = null;
  ...
  if (window.XMLHttpRequest)
	...
	var url = "http://localhost/wpmu41/uploadPhoto.php?car=" + car + "&post=" + post +"&blog=" + blog;
	req.open ("GET", url, true);
	req.send (null);
}
	if ((goal == 1) && (document.getElementById ("mode1").checked))
  {
  var req = null;
