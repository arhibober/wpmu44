		...
		if ( 'html5' == $format ) {
			$form = '<form role="search" method="get" class="search-form" action="' . esc_url( home_url( '/' ) ) . '">
				<label>
					...
					<input type="search" class="search-field" placeholder="' . esc_attr_x( 'Пошук &hellip;', 'placeholder' ) . '" value="' . get_search_query() . '" name="s" title="' . esc_attr_x( 'Шукати:', 'label' ) . '" />
				</label>
				<input type="submit" class="search-submit" value="'. esc_attr_x( 'Пошук', 'submit button' ) .'" />
			</form>';
		} else {
 ...
 */
function wp_editor( $content, $editor_id, $settings = array(), $isSwitch = true, $isPhoto = false, $post = 0, $blog = 0, $goal = "") {
	if ( ! class_exists( '_WP_Editors' ) )
		require( ABSPATH . WPINC . '/class-wp-editor.php' );

	_WP_Editors::editor($content, $editor_id, $settings, $isSwitch, $isPhoto, $post, $blog, $goal);
}
 ...
 */
function wp_admin_css_uri( $file = '' ) {
 ...
 */
function wp_admin_css( $file = '', $force_echo = false ) {
	global $wp_styles;
