...
class WP_Widget_Search extends WP_Widget {
	...
	function __construct() {
		...
		parent::__construct('search', __('Пошук'), $widget_ops);
	}
...
class WP_Widget_Archives extends WP_Widget {
	...
	function __construct() {
		...
		parent::__construct('archives', __($u.' - архів постів'), $widget_ops);
	}
	...
	function widget( $args, $instance ) {
		...
		$title = apply_filters('widget_title', empty($instance['title']) ? __($u.' - архів постів') : $instance['title'], $instance, $this->id_base);
		...
		if ( $d ) {
?>
		<select name="archive-dropdown" onchange='document.location.href=this.options[this.selectedIndex].value;'> <option value=""><?php echo esc_attr(__('Оберіть місяць')); ?></option> <?php wp_get_archives(apply_filters('widget_archives_dropdown_args', array('type' => 'monthly', 'format' => 'option', 'show_post_count' => $c))); ?> </select>
<?php
		} else {
?>
		<ul>
...
class WP_Widget_Meta extends WP_Widget {
	...
	function __construct() {
		...
		parent::__construct('meta', __('Мета'), $widget_ops);
	}

	function widget( $args, $instance ) {
		extract($args);
		$title = apply_filters('widget_title', empty($instance['title']) ? __('Мета') : $instance['title'], $instance, $this->id_base);
...
class WP_Widget_Categories extends WP_Widget {
	...
	function __construct() {
		...
		parent::__construct('categories', __($u.' - пости за категоріями'), $widget_ops);
	}
...
class WP_Widget_Recent_Posts extends WP_Widget {
	...
	function __construct() {
		...
		parent::__construct('recent-posts', __($u.' - нові пости'), $widget_ops);
	...
	function widget($args, $instance) {
		...
		$title = ( ! empty( $instance['title'] ) ) ? $instance['title'] : __( $u." - нові пости" );
...
class WP_Widget_Recent_Comments extends WP_Widget {
	...
	function __construct() {
		...
		parent::__construct('recent-comments', $u.' - нові коментарі', $widget_ops);
	...
	function widget( $args, $instance ) {
		...
		if ( $comments ) {
			...
			foreach ( (array) $comments as $comment) {
				$output .=  '<li class="recentcomments">' . /* translators: comments widget: 1: comment author, 2: post link */ sprintf(_x('%1$s на пост %2$s', 'widgets'), get_comment_author_link(), '<a href="' . esc_url( get_comment_link($comment->comment_ID) ) . '">' . get_the_title($comment->comment_post_ID) . '</a>') . '</li>';
			}
 		}
		$output .= '</ul>';
		$output .= $after_widget;
