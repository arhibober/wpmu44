...
function wpmu_validate_user_signup($user_name, $user_email) {
	...
	if ( empty( $user_name ) )
	   	$errors->add('user_name', __( 'Будь ласка, уведіть логін.' ) );
	...
	if ( strlen( $user_name ) < 4 )
		$errors->add('user_name',  __( 'Логін повинин містити як мінімум 4 сімвола.' ) );
	...
	if ( preg_match( '/^[0-9]*$/', $user_name ) )
		$errors->add('user_name', __('Вибачте, логін потрібен містити хоча б одну літеру!'));

	if ( !is_email( $user_email ) )
		$errors->add('user_email', __( 'Будь ласка, уведіть свій email.' ) );
	...
	if ( username_exists($user_name) )
		$errors->add( 'user_name', __( 'Пробачте, цей логін вже зарєєстрован на сайті!' ) );

	// Check if the email address has been used already.
	if ( email_exists($user_email) )
		$errors->add( 'user_email', __( 'Пробачте, цей e-mail вже зарєєстрован на сайті!' ) );
	...
	if ( $signup != null ) {
		...
		if ( $diff > 2 * DAY_IN_SECONDS )
			...
			$errors->add('user_name', __('Цей логін тимчасово зарезервований програмним забеспеченням сайту, але може бути активованим через кілька днів.'));
	}
	...
	if ( $signup != null ) {
		...
		if ( $diff > 2 * DAY_IN_SECONDS )
			...
			$errors->add('user_email', __('Цей e-mail вже зарєєстрован на цьому сайті. Будь ласка, первірте список листів на ньому. Якщо він не відновився, почекайте кілька днів.'));
	}
...
function wpmu_validate_blog_signup( $blogname, $blog_title, $user = '' ) {
	...
	if ( empty( $blogname ) )
		$errors->add('blogname', __( 'Будь ласка, уведіть адресу сайту.' ) );
	...
	if ( preg_match( '/^[0-9]*$/', $blogname ) )
		$errors->add('blogname', __('Пробачте, назва сайту повинна містити хоча ьб одну літеру!'));
	...
	if ( empty( $blog_title ) )
		$errors->add('blog_title', __( 'Будь ласка, уведіть заголовок сайту.' ) );
	...
	if ( domain_exists($mydomain, $path, $current_site->id) )
		$errors->add( 'blogname', __( 'Вибачте, блог з такою адресою вже існує!' ) );
...
function wpmu_signup_blog_notification( $domain, $path, $title, $user, $user_email, $key, $meta = array() ) {
	...
	$message = sprintf(
		...
		apply_filters( 'wpmu_signup_blog_notification_email',
			__( "Для актівації свого акаунту та автоматичної генерації його робочого паролю перейдіть, будь ласка, на посилання:\n\n%s\n\n" ),
...
function wpmu_signup_user_notification( $user, $user_email, $key, $meta = array() ) {
	...
	$message = sprintf(
		...
		apply_filters( 'wpmu_signup_user_notification_email',
			__( "Для активації Вашого акаунту та одночасної з нею автоматичною генерацією Вашого нового робочого паролю, перейдіть, буцдь ласка, по цьому посиланню:\n\n%s\n\n." ),
