...
function wp_page_menu( $args = array() ) {
	...
	$list_args = $args;
	$menu .= '<li><a href="http://localhost/wpmu41/додати-новий-пост/?type=abc&page1=0">Всі блоги</a></li>&nbsp;';
	$menu .= '<li><a href="http://localhost/wpmu41">До головної сторінки мережі</a></li>&nbsp;';
	global $user_ID;
	if ($user_ID > 1)
	{
		global $wpdb;
		...
		$menu .= '<li><a href="http://localhost'.$sites [0]["path"].'">До Вашого блогу</a></li>&nbsp;';
	}
	global $blog_id;
	...
	if ( ! empty($args['show_home']) ) {
		...
		$name1 = $wpdb->get_results ($sqlstr, ARRAY_A);
		if ( true === $args['show_home'] || '1' === $args['show_home'] || 1 === $args['show_home'] )
			$text = __('До стрічки користувача '.$name1 [0]["path"]);
		else
			$text = $args['show_home'];
		$class = '';
		...
		if ($blog_id > 1)
			$menu .= '<li ' . $class . '><a href="' . home_url( '/' ) . '">До стрічки користувача '.substr ($name1 [0]["path"], 8, strlen ($name1 [0]["path"]) - 9).'</a></li>';
