...
function wp_admin_bar_my_account_item( $wp_admin_bar ) {
	...
	$howdy  = sprintf( __('Привіт, %1$s'), $current_user->display_name );
...
function wp_admin_bar_my_account_menu( $wp_admin_bar ) {
	...
	$wp_admin_bar->add_menu( array(
		'parent' => 'user-actions',
		'id'     => 'edit-profile',
		'title'  => __( 'Налаштування особистого профайлу' ),
		'href' => $profile_url,
	) );
	$wp_admin_bar->add_menu( array(
		'parent' => 'user-actions',
	...
	foreach ( (array) $wp_admin_bar->user->blogs as $blog ) {
		...
		if ( current_user_can( 'edit_posts' ) ) {
			$wp_admin_bar->add_menu( array(
				'parent' => $menu_id,
				'id'     => $menu_id . '-c',
				'title'  => __( 'Керувати коментаріями' ),
...
function wp_admin_bar_new_content_menu( $wp_admin_bar ) {
	...
	if ( isset( $cpts['post'] ) && current_user_can( $cpts['post']->cap->create_posts ) )
		$actions[ 'post-new.php' ] = array( _x( 'Пост', 'add new from admin bar' ), 'new-post' );

	if ( isset( $cpts['attachment'] ) && current_user_can( 'upload_files' ) )
		$actions[ 'media-new.php' ] = array( _x( 'Мультімедійні дані', 'add new from admin bar' ), 'new-media' );

	if ( current_user_can( 'manage_links' ) )
		$actions[ 'link-add.php' ] = array( _x( 'Посилання', 'add new from admin bar' ), 'new-link' );

	if ( isset( $cpts['page'] ) && current_user_can( $cpts['page']->cap->create_posts ) )
		$actions[ 'post-new.php?post_type=page' ] = array( _x( 'Одиночна сторінка', 'add new from admin bar' ), 'new-page' );
	...
	if ( current_user_can( 'create_users' ) || current_user_can( 'promote_users' ) )
		$actions[ 'user-new.php' ] = array( _x( 'Користувач', 'add new from admin bar' ), 'new-user' );

	if ( ! $actions )
		return;

	$title = '<span class="ab-icon"></span><span class="ab-label">' . _x( 'Новий', 'admin bar menu group label' ) . '</span>';
	...
	$wp_admin_bar->add_menu( array(
		...
		'meta'  => array(
			'title' => _x( 'Додати новий', 'admin bar menu group label' ),
		),
	) );
...
function wp_admin_bar_comments_menu( $wp_admin_bar ) {
	...
	$awaiting_title = esc_attr( sprintf( _n( '%s коментарій очікує модерації', '%s коментаріїв очікує модерації', $awaiting_mod ), number_format_i18n( $awaiting_mod ) ) );
...
function wp_admin_bar_appearance_menu( $wp_admin_bar ) {
	...
	if ( current_user_can( 'switch_themes' ) || current_user_can( 'edit_theme_options' ) )
		$wp_admin_bar->add_menu( array( 'parent' => 'appearance', 'id' => 'themes', 'title' => __('Теми'), 'href' => admin_url('themes.php') ) );
	...
	if ( current_theme_supports( 'widgets' )  )
		$wp_admin_bar->add_menu( array( 'parent' => 'appearance', 'id' => 'widgets', 'title' => __('Віджети'), 'href' => admin_url('widgets.php') ) );

	if ( current_theme_supports( 'menus' ) || current_theme_supports( 'widgets' ) )
		$wp_admin_bar->add_menu( array( 'parent' => 'appearance', 'id' => 'menus', 'title' => __('Керування меню сайту'), 'href' => admin_url('nav-menus.php') ) );

	if ( current_theme_supports( 'custom-background' ) )
		$wp_admin_bar->add_menu( array( 'parent' => 'appearance', 'id' => 'background', 'title' => __('Фон'), 'href' => admin_url('themes.php?page=custom-background') ) );
