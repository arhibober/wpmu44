...
function get_comment_date( $d = '', $comment_ID = 0 ) {
	$comment = get_comment( $comment_ID );
	//if ( '' == $d )
		$date = mysql2date("d.m.y H:i", $comment->comment_date);
	/*else
...
function comment_type( $commenttxt = false, $trackbacktxt = false, $pingbacktxt = false ) {
	if ( false === $commenttxt ) $commenttxt = _x( 'Залишити коментарій', 'noun' );
...
function comments_popup_link( $zero = false, $one = false, $more = false, $css_class = '', $none = false ) {
	...
	echo ' title="' . esc_attr( sprintf( __('Комментарі до посту'), $title ) ) . '">';
...
function comment_form_title( $noreplytext = false, $replytext = false, $linktoparent = true ) {
	...
	if ( false === $noreplytext ) $noreplytext = __( 'Залешити коментарій' );
...
?>
		...
		<div class="comment-author vcard">
			...
			<?php printf( __( '<cite class="fn">%s</cite> <span class="says">написав:</span>' ), get_comment_author_link() ); ?>
		</div>
		...
		<div class="comment-meta commentmetadata"><a href="<?php echo esc_url( get_comment_link( $comment->comment_ID ) ); ?>">
			<?php
				...
				printf( __( '%1$s' ), get_comment_date() ); ?></a><?php edit_comment_link( __( '(Редагувати)' ), '&nbsp;&nbsp;', '' );
			?>
		</div>
		...
		<<?php echo $tag; ?> id="comment-<?php comment_ID(); ?>" <?php comment_class( empty( $args['has_children'] ) ? '' : 'parent' ); ?>>
			<article id="div-comment-<?php comment_ID(); ?>" class="comment-body">
				<footer class="comment-meta">
					<div class="comment-author vcard">
						<?php if ( 0 != $args['avatar_size'] ) echo get_avatar( $comment, $args['avatar_size'] ); ?>
						<?php printf( __( '%s <span class="says">написав:</span>' ), sprintf( '<b class="fn">%s</b>', get_comment_author_link() ) ); ?>
					</div><!-- .comment-author -->
					...
					<div class="comment-metadata">
						...
						<?php edit_comment_link( __( 'Редагувати' ), '<span class="edit-link">', '</span>' ); ?>
...
function comment_form( $args = array(), $post_id = null ) {
	...
	$fields   =  array(
		'author' => '<p class="comment-form-author">' . '<label for="author">' . __( 'Логін' ) . ( $req ? ' <span class="required">*</span>' : '' ) . '</label> ' .
	...
	$required_text = sprintf( ' ' . __('Обов\'язкові поля помічені %s'), '<span class="required">*</span>' );
	...
	$defaults = array(
		'fields'               => $fields,
		'comment_field'        => '<p class="comment-form-comment"><label for="comment">' . _x( 'Коментарій', 'noun' ) . '</label> <textarea id="comment" name="comment" cols="45" rows="8" aria-required="true"></textarea></p>',
		'must_log_in'          => '<p class="must-log-in">' . sprintf( __( 'Щоб залишити коментарій, будь ласка, <a href="%s">зайдіть</a> на сайт.' ), wp_login_url( apply_filters( 'the_permalink', get_permalink( $post_id ) ) ) ) . '</p>',
		'logged_in_as'         => '<p class="logged-in-as">' . sprintf( __( 'Ви зайшли як <a href="%1$s">%2$s</a>. <a href="%3$s" title="Log out of this account">Вийти?</a>' ), get_edit_user_link(), $user_identity, wp_logout_url( apply_filters( 'the_permalink', get_permalink( $post_id ) ) ) ) . '</p>',
		'comment_notes_before' => '<p class="comment-notes">' . __( 'Ваш e-mail не буде видно іншим користувачам сайту.' ) . ( $req ? $required_text : '' ) . '</p>',
		'comment_notes_after'  => '',
		'id_form'              => 'commentform',
		'id_submit'            => 'submit',
		'title_reply'          => __( 'Залишити коментарій' ),
		'title_reply_to'       => __( 'Leave a Reply to %s' ),
		'cancel_reply_link'    => __( 'Скасувати' ),
			...
			<div id="respond" class="comment-respond">
				...
				<?php else : ?>
					<form action="<?php echo site_url( '/wp-comments-post.php' ); ?>" method="post" id="<?php echo esc_attr( $args['id_form'] ); ?>" class="comment-form"<?php echo $html5 ? ' novalidate' : ''; ?>>
						...
						<p class="form-submit">
							<input name="submit" type="submit" id="<?php echo esc_attr( $args['id_submit'] ); ?>" value="<?php echo esc_attr( $args['label_submit'] ); ?>" class="buttons" style="font-size: 12px !important;"/>
