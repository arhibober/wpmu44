...
function get_category_parents( $id, $link = false, $separator = '/', $nicename = false, $visited = array() ) {
	...
	if ( $link )
		$chain .= '<a href="' . esc_url( get_category_link( $parent->term_id ) ) . '" title="' . esc_attr( sprintf( __( "Інші пости в категорії %s" ), $parent->name ) ) . '">'.$name.'</a>' . $separator;
	else
		...
		foreach ( $categories as $category ) {
			...
			switch ( strtolower( $parents ) ) {
				case 'multiple':
					...
					$thelist .= '<a href="' . esc_url( get_category_link( $category->term_id ) ) . '" title="' . esc_attr( sprintf( __( "Інші поси в категорії %s" ), $category->name ) ) . '" ' . $rel . '>' . $category->name.'</a></li>';
					break;
				case 'single':
					$thelist .= '<a href="' . esc_url( get_category_link( $category->term_id ) ) . '" title="' . esc_attr( sprintf( __( "Інші пости в категорії %s" ), $category->name ) ) . '" ' . $rel . '>';
					if ( $category->parent )
				...
				case '':
				default:
					$thelist .= '<a href="' . esc_url( get_category_link( $category->term_id ) ) . '" title="' . esc_attr( sprintf( __( "Інші пости в категорії %s" ), $category->name ) ) . '" ' . $rel . '>' . $category->name.'</a></li>';
			}
		}
		$thelist .= '</ul>';
	} else {
		...
		foreach ( $categories as $category ) {
			...
			switch ( strtolower( $parents ) ) {
				case 'multiple':
					if ( $category->parent )
						$thelist .= get_category_parents( $category->parent, true, $separator );
					$thelist .= '<a href="' . esc_url( get_category_link( $category->term_id ) ) . '" title="' . esc_attr( sprintf( __( "Інші пости в категорії %s" ), $category->name ) ) . '" ' . $rel . '>' . $category->name.'</a><br/>';
					break;
				case 'single':
					$thelist .= '<a href="' . esc_url( get_category_link( $category->term_id ) ) . '" title="' . esc_attr( sprintf( __( "Інші пости в категорії %s" ), $category->name ) ) . '" ' . $rel . '>';
					if ( $category->parent )
						$thelist .= get_category_parents( $category->parent, false, $separator );
					$thelist .= "$category->name</a><br/>";
					break;
				case '':
				default:
					$thelist .= '<a href="' . esc_url( get_category_link( $category->term_id ) ) . '" title="' . esc_attr( sprintf( __( "Інші пости в категорії %s" ), $category->name ) ) . '" ' . $rel . '>' . $category->name.'</a><br/>';
...
function wp_list_categories( $args = '' ) {
	$defaults = array(
		'show_option_all' => '', 'show_option_none' => __('Нема категорій'),
...
class Walker_Category extends Walker {
	...
	function start_el( &$output, $category, $depth = 0, $args = array(), $id = 0 ) {
		...
		if ( $use_desc_for_title == 0 || empty($category->description) )
			$link .= 'title="' . esc_attr( sprintf(__( 'Інші пости в категорії %s' ), $cat_name) ) . '"';
		else
