...
function show_blog_form( $blogname = '', $blog_title = '', $errors = '' ) {
	...
	if ( !is_subdomain_install() )
		echo '<label for="blogname">' . __('Назва блогу:') . '</label>';
	else
...
"guid" => "http://localhost/wpmu41/".$sites [0]["meta_value"]."/?p=".($max_id [0]["max_id"] + 1)));
	...
	// Blog Title
	?>
	<label for="blog_title"><?php _e('Заголовок сайту:') ?></label>
	...
	<div id="privacy">
        <p class="privacy-intro">
            <label for="blog_public_on"><?php _e('Приватність:') ?></label>
...
function show_user_form($user_name = '', $user_email = '', $errors = '') {
	// User name
	echo '<label for="user_name">' . __('Логін:') . '</label>';
	if ( $errmsg = $errors->get_error_message('user_name') ) {
		echo '<p class="error">'.$errmsg.'</p>';
	}
	echo '<input name="user_name" type="text" id="user_name" value="'. esc_attr($user_name) .'" maxlength="60" /><br />';
	_e( '(Повинен містити тільки літери та цифри, в сумі мінімум 4 символи.)' );
	?>
	...
	<?php if ( $errmsg = $errors->get_error_message('user_email') ) { ?>
		<p class="error"><?php echo $errmsg ?></p>
	<?php } ?>
	<input name="user_email" type="text" id="user_email" value="<?php  echo esc_attr($user_email) ?>" maxlength="200" /><br /><?php _e('
На цей e-mail вам повинне буде прийти посилання на актівацію Вашого акаунту та генерацію робочого паролю, тому адреса повинна бути реальною.') ?>
	<?php
...
function signup_another_blog( $blogname = '', $blog_title = '', $errors = '' ) {
	...
	echo '<h2>' . sprintf( __( 'Зараз Ви можете створити <em>нову</em> блогову стрічку буквально за кілька секунд' ), get_current_site()->site_name ) . '</h2>';
	?>
...
function confirm_another_blog_signup( $domain, $path, $blog_title, $user_name, $user_email = '', $meta = array() ) {
	?>
	<h2><?php printf( __( 'Вітаємо, Ваш новий блог %s відкрит в мережі успішно!' ), "<a href='http://{$domain}{$path}'>{$blog_title}</a>" ) ?></h2>
	<p>
		<?php printf( __( 'Ви створили свій новий блог з адресою <a href="http://%1$s">http://%2$s</a>. Для керування їм Ви можете <a href="%3$s">увійти</a> на нього під своїм основним акаунтом.' ), $domain.$path, $domain.$path, "http://" . $domain.$path . "wp-login.php", $user_name ) ?>
	</p>
	<?php
	/**
...
function signup_user( $user_name = '', $user_email = '', $errors = '' ) {
	...
	<h2><?php printf( __( '<br/>Зареєструйте свій аккаунт буквально за кілька секунд' ), get_current_site()->site_name ) ?></h2>
	<form id="setupform" method="post" action="wp-signup.php">
...
function confirm_user_signup($user_name, $user_email) {
	?>
	<h2><?php printf( __( '<br/>
	Ви успішно зарєєстрували на сайті свій акаунт %s.' ), $user_name) ?></h2>
	<p><?php _e( 'Але, під початком використання його функціональності, будь ласка, <strong>активуйте його</strong>.' ) ?></p>
	<p><?php printf( __( 'Посилання на його активізацію, а також на його майбутній робочий пароль. відправлена Вам на e-mail <strong>%s</strong>.' ), $user_email ); ?></p>
	<?php
	/** This action is documented in wp-signup.php */
	do_action( 'signup_finished' );
}
...
function signup_blog($user_name = '', $user_email = '', $blogname = '', $blog_title = '', $errors = '') {
	...
	<form id="setupform" method="post" action="wp-signup.php">
		...
		?>
		<?php show_blog_form($blogname, $blog_title, $errors); ?>
		<p class="submit"><input type="submit" name="submit" class="buttons" value="<?php esc_attr_e('Підтвердити') ?>" style="padding: 8px 6px 8px 6px !important;"/></p>
	</form>
	<?php
}
...
function confirm_blog_signup( $domain, $path, $blog_title, $user_name = '', $user_email = '', $meta = array() ) {
	?>
	<h2><?php printf( __( 'Вітаємо! Ваш новий сайт, %s, вже майже готовий.' ), "<a href='http://{$domain}{$path}' style='color: #004369 !important;'>{$blog_title}</a>" ) ?></h2>

	<p><?php _e( 'Але перед перехідом до його функціональності, Вам необхідно <strong>актувати його</strong>.' ) ?></p>
	<p><?php printf( __( 'Посилання, ведуче на цю активацію, а також одночасно на сторінку з автоматично генерируємим його робочим паролем, переслано на сайт	<strong>%s</strong>.' ),  $user_email) ?></p>
	<?php
	/** This action is documented in wp-signup.php */
	do_action( 'signup_finished' );
}
