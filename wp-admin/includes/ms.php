...
function _access_denied_splash() {
	...
	if ( empty( $blogs ) )
		wp_die( sprintf( __( 'На жаль, запитувана Вами функція недоступна щодо користувачів Вашого рівня.' ), $blog_name ) );

	$output .= 'На жаль, запитувана Вами функція недоступна щодо користувачів Вашого рівня.<br/>
	<h3>' . __('Ваши сайти') . '</h3>';
	$output .= '<table>';
	...
	foreach ( $blogs as $blog ) {
		...
		$output .= "</td>";
		$output .= "<td valign='top'>";
		$output .= "<a href='" . esc_url( get_home_url( $blog->userblog_id ) ). "'>" . __( 'Перейти' ) . "</a>" ;
		$output .= "</td>";
		$output .= "</tr>";
	}
	$output .= '</table>';
